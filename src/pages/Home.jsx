...
export default function Home() {
  ...
  return (
    <div className="page">
      ...
      <p style={{ color: '#b8849a', marginBottom: '2rem', fontSize: 16 }}>
      ...
      <div className="grid-3">
        {CARDS.map(c => (
          <div
            ...
            onMouseEnter={e => e.currentTarget.style.boxShadow = '0 4px 16px rgba(224,112,144,0.12)'}
            onMouseLeave={e => e.currentTarget.style.boxShadow = 'none'}
          >
            <div style={{ fontSize: 28, marginBottom: 10 }}>{c.icon}</div>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 4, color: '#2d1f26' }}>{c.label}</div>
            <div style={{ fontSize: 13, color: '#b8849a' }}>{c.desc}</div>
          </div>
        ))}
      </div>
    </div>
